...
import { ABOUT_ROUTE, ADMIN_LOGIN_ROUTE, BASE_ROUTE, CONTACT_ROUTE, USER_LOGIN_ROUTE, USER_SIGNUP_ROUTE } from "../constants/AppRoutes";
import { useLocation, useNavigate } from "react-router-dom";
import {getToken, removeUserId, removeToken, removeUserType} from '../services/authServices'
  ...
  useEffect(() => {
    ...
    switch (userType) {
      case "User":
        ...
        break;
      case "Admin":
        if (location.pathname === `${USER_LOGIN_ROUTE}`) {
          navigate(`${ADMIN_LOGIN_ROUTE}`);
        }
        if (location.pathname === `${USER_SIGNUP_ROUTE}`) {
          navigate(`${ADMIN_LOGIN_ROUTE}`);
        }
        break;
      default:
